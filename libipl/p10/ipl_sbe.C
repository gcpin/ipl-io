...
ipl_error_type ipl_sbe_mpipl_continue(TARGETING::ConstTargetPtr target)
{
	...
	//TODO ipl_log(IPL_INFO, "ipl_sbe_mpipl_continue: Enter(%s)", pdbg_target_path(proc));

    rc = ipl_sbe_get_state(target, &state);

    if (rc != 0)
    {
		//TODO ipl_log(IPL_ERROR, "Failed to read SBE state information (%s)", pdbg_target_path(pib));
	...
	if (rc != 0)
    {
		//TODO ipl_log(IPL_ERROR, "SBE (%s) mpipl continue chip-op failed", pdbg_target_path(pib));
		return IPL_ERR_SBE_CHIPOP;
	}
...
bool ipl_sbe_booted(TARGETING::TargetPtr target, uint32_t wait_time_seconds)
{
	...
	while (loopcount > 0)
    {
        std::cout << "DEMO: executing p10_get_sbe_msg_register\n";
	    fapi_rc = p10_get_sbe_msg_register(target, sbeReg);
        std::cout << "DEMO: done p10_get_sbe_msg_register\n";
        ...
        if (fapi_rc == fapi2::FAPI2_RC_SUCCESS)
        {
            ...
            {
                std::cout << "DEMO: SBE Boot is in progress\n";
				ipl_log(
				    IPL_DEBUG,
        ...
        else
        {
            ...
            std::cout << "DEMO: p10_get_sbe_msg_register failed\n";
			/*TODO ipl_log(IPL_ERROR,
